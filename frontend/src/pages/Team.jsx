...
const Team = () => {
  ...
  const filteredUploads = uploads?.filter((upload) => upload.type === "team");
  if (isLoading) return <div>Loading...</div>;
  if (error) return <div>Error loading uploads.</div>;

  return (
    <div id="team" className="relative w-full min-h-screen overflow-hidden">
      <img
        src={bgImage}
        alt="Background"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4 w-full max-w-7xl">
          {filteredUploads?.map((upload) => (
            <div
              ...
              className={`flex flex-col items-center justify-center p-4 rounded-lg h-60 bg-gradient-to-b from-sky-300 to-white `}
            >
              {upload.imageUrl && (
                <img
                  src={upload.imageUrl}  
                  alt={upload.heading || "Team Image"}
